...
int OpenComport2(const char* port, int baudrate)
{
  int baudr;
  ...
  C_port = open(port, O_RDWR | O_NOCTTY | O_NDELAY);
  if(C_port==-1)
  {
    perror("unable to open comport ");
    return(1);
  ...
  //error = tcgetattr(C_port, old_port_settings + comport_number);
  if(error==-1)
  {
    close(C_port);
    perror("unable to read portsettings ");
  ...
  new_port_settings.c_cc[VMIN] = 0;      /* block untill n bytes are received */
  new_port_settings.c_cc[VTIME] = 0;     /* block untill a timer expires (n * 100 mSec.) */
  error = tcsetattr(C_port, TCSANOW, &new_port_settings);
  if(error==-1)
  {
    close(C_port);
    perror("unable to adjust portsettings ");
...
int PollComport2(unsigned char *buf, int size)
{
  int n;
...
int SendByte2(unsigned char byte)
{
  int n;
...
int SendBuf2(unsigned char *buf, int size)
{
  return(write(C_port, buf, size));
}
